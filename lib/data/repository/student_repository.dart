...
class StudentRepository {
  ...
  Future<StudentModel> getStudentById(String studentId) async {
    return await _fireStore.collection(StringConstants.users).doc(studentId).get().then((value) {
  ...
  Future<bool> addStudent(StudentModel studentModel, String password) async {
    ...
    var username = '${'${x.id}@${studentModel.orgId}'}.eliteacademy.co.in';

    var cred = await auth.createUserWithEmailAndPassword(email: username, password: password);
    await auth.signOut();
    await auth.signInWithEmailAndPassword(
      email: "ps@eliteacademy.co.in",
      password: "150783Pn@",
    );
  ...
  Future<void> deleteStudent(StudentModel studentModel) async {
    await _fireStore.collection(StringConstants.users).doc(studentModel.id).delete();
  }
}
