...
class AdminRepository {
  ...
  Future<AdminModel> getAdminById(String adminId) async {
    return await _fireStore.collection(StringConstants.users).doc(adminId).get().then((value) {
      return AdminModel.fromMap(value.data()!);
    });
  }

  Future<void> addAdmin(AdminModel adminModel) async {
    DocumentReference docRef = _fireStore.collection(StringConstants.users).doc();
  ...
  Future<void> createAdmin(AdminModel adminModel) async {
    ...
    DocumentReference docRef = _fireStore.collection(StringConstants.users).doc(adminModel.id);
  ...
  Future<void> deleteAdmin(AdminModel adminModel) async {
    await _fireStore.collection(StringConstants.users).doc(adminModel.id).delete();
  }
}
